...
import example.spring.batch.component.JobCompletionNotificationListener;
import example.spring.component.PurchaseOrderIdGenerator;
import example.spring.component.PurchaseOrderProcessor;
import example.spring.domain.PurchaseOrder;

@Configuration
@EnableBatchProcessing
@ComponentScan(basePackages = {"example.spring.batch.component"})
public class BatchConfiguration {
	...
	public Job importPurchaseOrderJob(JobCompletionNotificationListener listener) {
		return jobBuilderFactory.get("importPurchaseOrderJob").incrementer(new RunIdIncrementer()).listener(listener)
				.flow(step1()).end().build();
	}

	@Bean
	public Step step1() {
		return stepBuilderFactory.get("step1").<PurchaseOrder, PurchaseOrder> chunk(10).reader(reader())
				.processor(getPurchaseOrderProcessor()).writer(writer()).build();
	}
    ...
    public DataSource dataSource() {
        return new EmbeddedDatabaseBuilder()
            ...
            .addScript("classpath:data-all.sql")
